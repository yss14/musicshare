...
export type TypedRequestMiddleware<R, T> = (
	request: R
...
export const withMiddleware: {
	...
	<R, T, K, V>(m1: TRM<R, T>, m2: TRM<R, K>, m3: TRM<R, V>): (handler: (req: R, v1: T, v2: K, v3: V) => Promise<IResponse>) => CustomExpressRequestHandler<R>,
	<R, T, K, V, S>(m1: TRM<R, T>, m2: TRM<R, K>, m3: TRM<R, V>, m4: TRM<R, S>): (handler: (req: R, v1: T, v2: K, v3: V, v4: S) => Promise<IResponse>) => CustomExpressRequestHandler<R>,
} = <R, T, K, V, S>(m1: TRM<R, T>, m2?: TRM<R, K>, m3?: TRM<R, V>, m4?: TRM<R, S>) => (handler: (req: R, v1: T, v2?: K, v3?: V, v4?: S) => Promise<IResponse>): CustomExpressRequestHandler<R> => {
	return async (req: R) => {
											...
											if (m4) {
												m4(req).then(response4 => {
													if (isLeft(response4)) {
														...
														handler(req, response1.right, response2.right, response3.right, response4.right)
															.then(resolve, reject);
													}
												}, reject);
											} else {
												handler(req, response1.right, response2.right, response3.right)
													.then(resolve, reject);
											}
										}
									}, reject);
								} else {
									handler(req, response1.right, response2.right)
										.then(resolve, reject);
								}
							}
						}, reject);
					} else {
						handler(req, response1.right)
							.then(resolve, reject);
					}
				}
			}, reject);
