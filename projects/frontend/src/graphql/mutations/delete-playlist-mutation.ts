...
export const useDeletePlaylist = (opts?: IMutationOptions<IDeletePlaylistData>) => {
	const [deletePlaylistMutation, other] = useMutation<IDeletePlaylistData, IDeletePlaylistVariables>(DELETE_PLAYLIST, opts)

	const makeUpdateCache = useCallback((shareID: string, playlistID: string): MutationUpdaterFn<IDeletePlaylistData> =>
		(cache, { data }) => {
	...
	const deletePlaylist = useCallback((shareID: string, playlistID: string) => {
