...
import { IUploadSchema, UploadItemStatus } from "../../graphql/rest-types"
	...
	switch (action.type) {
		...
		case UPLOAD_STARTED:
			return state.map((upload) =>
				upload.hash === action.payload.fileHash ? { ...upload, status: UploadItemStatus.Uploading } : upload,
			)

		case UPLOAD_PROGRESS:
			return state.map((upload) =>
				upload.hash === action.payload.fileHash ? { ...upload, progress: action.payload.progress } : upload,
			)
