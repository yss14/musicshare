...
export class ArtistExtractor {
	...
	private readonly separators = ArtistExtractor.featuringSeparators.concat([
		";",
		"vs.",
		"Vs.",
		"VS.",
	...
	public extract(artistStr: string, artistType: ArtistType): IArtist[] {
		...
		for (const separator of this.separators) {
			if (newArtistStr.indexOf(separator) > -1) {
				...
				const newArtistType = (artistSubstr: string): ArtistType =>
					ArtistExtractor.featuringSeparators.includes(separator) &&
					artistStr.indexOf(artistSubstr) >= featuringIndex
						? "featurings"
						: artistType

				return cutArtists.concat(_.flatMap(split.map((splt) => this.extract(splt, newArtistType(splt)))))
			}
		}

		const artName = newArtistStr
			.trim()
			.replace(/\0/g, "")
			.split("(")
			.join("")
	...
	private cutKnownArtists(artistStr: string, artistType: ArtistType): [IArtist[], string] {
		...
		for (const knownArtist of this.knownArtists) {
			...
			if (startIndex > -1 && startIndex < featuringIndex) {
				...
				if (
					!this.separators.some((separator) => newArtistStr.indexOf(separator) > -1) &&
					newArtistStr.trim().length > 0
				) {
