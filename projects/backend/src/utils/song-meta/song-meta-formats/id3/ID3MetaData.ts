...
export class ID3MetaData implements ISongMetaDataSource {
	constructor(
		private readonly artistExtractor: ArtistExtractor
	) { }
	...
	public async analyse(file: IFile, audioBuffer: Buffer, songTypes: ISongType[]): Promise<ExtractedSongMetaData> {
		...
		try {
			...
			if (id3Tags && id3Tags.title !== undefined && id3Tags.title.trim().length > 0 && isValidTitle(id3Tags.title.trim())) {
			...
			if (songtype) {
				...
				if (type.hasArtists) {
					...
					if (typeDelimiterChar === '') {
						idxArtistBegin = this.indexOfCharLeft(title, '(', idxTypeBegin - 1) ? this.indexOfCharLeft(title, '(', idxTypeBegin - 1) :
							(this.indexOfCharLeft(title, '[', idxTypeBegin - 1) > -1 ? this.indexOfCharLeft(title, '[', idxTypeBegin - 1) : -1);
					} else {
	...
	private getArtists(id3Tags: IID3Tag): IArtist[] {
		...
		if (id3Tags.artist !== undefined) {
			this.artistExtractor.extract(id3Tags.artist, 'artists')
			...
			for (const textInformation of userDefinedTextInfo) {
				if (textInformation.description && textInformation.description.toLowerCase() === 'artists') {
					if (textInformation.value) {
						this.artistExtractor.extract(textInformation.value, 'artists')
							.forEach((artist => artistNames.add(artist.name)));
					}
				}
			}
