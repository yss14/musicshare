...
export class AWSS3FileService implements IFileService {
	...
	public getLinkToFile({ expireDate, filenameRemote, ipAddress }: GetLinkToFileArgs): Promise<string> {
		return new Promise<string>((resolve, reject) => {
			const expireSecond = Math.round((moment(expireDate || moment().add(10, 'minutes')).valueOf() - Date.now()) / 1000);

			this.s3Client.getSignedUrl('getObject', { Bucket: this.bucket, Key: filenameRemote, Expires: expireSecond }, (err, url) => {
				// istanbul ignore if
				if (err) return reject(err);
