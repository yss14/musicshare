...
describe("add songs to playlist", () => {
	...
	test("existing songs", async () => {
		...
		const query = makeMutation(
			makeAddSongsQuery(
				shareID,
				playlistID,
				songs.map((song) => song.song_id.toString()),
			),
		)
...
describe("remove songs from playlist", () => {
	...
	test("existing songs", async () => {
		...
		const query = makeMutation(
			makeRemoveSongsQuery(
				shareID,
				playlistID,
				songs.map((song) => song.playlistSongID),
			),
		)
	...
	test("not existing songs", async () => {
		...
		expect(body.data.removeSongsFromPlaylist).toBeArrayOfSize(
			testData.playlists.playlist1_library_user1.songs.length,
		)
	})
...
describe("update order", () => {
	const makeUpdateOrderMutation = (shareID: string, playlistID: string, orderUpdates: OrderUpdate[]) => `
		...
		.map((orderUpdate) => `["${orderUpdate[0]}", ${orderUpdate[1]}]`)
		.join(",")}]){
			${playlistSongKeys}
		}
	`
	...
	test("valid order", async () => {
		...
		expect(body.data.updateOrderOfPlaylist.map(mapPlaylistSongID)).toEqual(
			[playlistSongs[2], playlistSongs[1], playlistSongs[0]].map(mapPlaylistSongID),
		)
	})
	...
	test("not existing songs", async () => {
		...
		expect(body).toMatchObject(makeGraphQLResponse(null, [{ message: `Some songs are not part of this playlist` }]))
	})
