...
test("one parameter success", async () => {
	const testRoute = wrapRequestHandler(
		withMiddleware(firstMiddleware)(async (req, v1) => ResponseSuccessJSON(HTTPStatusCodes.OK, { v1 })),
	)
	...
	const httpResponse = await supertest(expressApp).post("/testroute").send({ someNumber: 42 })
...
test("two parameters failing", async () => {
	...
	const httpResponse = await supertest(expressApp).post("/testroute").send({ someNumber: 42, someString: false })
...
test("catching unhandled error", async () => {
	...
	const httpResponse = await supertest(expressApp).post("/testroute").send()
