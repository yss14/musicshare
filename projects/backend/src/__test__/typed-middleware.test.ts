...
const firstMiddlewareError = {
	identifier: "1",
	message: "<someNumber> is not a valid number",
};
const firstMiddleware = async (
	req: express.Request
): Promise<Either<IResponse, number>> => {
	...
	if (typeof someNumber === "number") {
		...
		return left(
			ResponseError(HTTPStatusCodes.BAD_REQUEST, firstMiddlewareError)
		);
	}
};

const secondMiddlewareError = {
	identifier: "2",
	message: "<someString> is not a valid number",
};
const secondMiddleware = async (
	...
	if (typeof someString === "string") {
		...
		return left(
			ResponseError(HTTPStatusCodes.BAD_REQUEST, secondMiddlewareError)
		);
	}
};

const thirdMiddlewareError = {
	identifier: "3",
	message: "<someBoolean> is not a valid number",
};
const thirdMiddleware = async (
	req: express.Request
): Promise<Either<IResponse, boolean>> => {
	...
	if (typeof someBoolean === "boolean") {
		...
		return left(
			ResponseError(HTTPStatusCodes.BAD_REQUEST, thirdMiddlewareError)
		);
	}
};
...
test("one parameter success", async () => {
	const testRoute = wrapRequestHandler(
		withMiddleware(firstMiddleware)(async (req, v1) =>
			ResponseSuccessJSON(HTTPStatusCodes.OK, { v1 })
		)
	);
...
test("two parameters success", async () => {
	const testRoute = wrapRequestHandler(
		withMiddleware(firstMiddleware, secondMiddleware)(async (req, v1, v2) =>
			ResponseSuccessJSON(HTTPStatusCodes.OK, { v1, v2 })
		)
	);
	const router = express.Router().post("/testroute", testRoute);
	const expressApp = makeExpressApp({
...
test("three parameters success", async () => {
	const testRoute = wrapRequestHandler(
		withMiddleware(firstMiddleware, secondMiddleware, thirdMiddleware)(
			// tslint:disable:max-func-args
			async (req, v1, v2, v3) =>
				ResponseSuccessJSON(HTTPStatusCodes.OK, { v1, v2, v3 })
		)
	);
	const router = express.Router().post("/testroute", testRoute);
	const expressApp = makeExpressApp({
