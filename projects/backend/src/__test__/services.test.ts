...
describe("file storage", () => {
	...
	describe("aws s3", () => {
		test("no s3 credentials fails", () => {
			const newConfig: IConfig = {
				...config,
				fileStorage: { ...config.fileStorage, s3: undefined, provider: "awss3" },
			}

			expect(() => initServices(newConfig, makeMockedDatabase())).toThrowError(
				"AWS S3 is specified as file storage provider, but no credentials are provided",
			)
		})
	})
	...
	test("unknown file storage providers fails", () => {
		const newConfig: IConfig = {
			...config,
			fileStorage: { ...config.fileStorage, s3: undefined, provider: <any>"some_other" },
		}

		expect(() => initServices(newConfig, makeMockedDatabase())).toThrowError(
			"Unknown file storage provider some_other",
		)
	})
})
